...
def get_eval_images(web_dir):
    old_path = web_dir + "/images/"
    new_path = web_dir + "/for_evaluation/"
    if os.path.isdir(new_path) is False:
        os.mkdir(new_path)
    imgs = os.listdir(old_path)
    for img in imgs:
        if "fake_B" in img:
            os.rename(old_path + img, new_path + img)
...
def train(opt):
    ...
    for epoch in range(opt.epoch_count,
        ...
        for i, data in enumerate(dataset):  # inner loop within one epoch
            ...
            #     print(f"Iteration time: {t_data} seconds")
