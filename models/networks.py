...
def define_G(input_nc, output_nc, ngf, netG, norm='batch', use_dropout=False, init_type='normal', init_gain=0.02,
    ...
    elif netG == 'calimar_gan_A':
        net = CalimarGan_Generator_A(input_nc, output_nc, ngf, n_blocks=5)
    elif netG == 'calimar_gan_B':
        net = CalimarGan_Generator_B(input_nc, output_nc, ngf, n_blocks=5)
    else:
...
class CalimarGan_Generator_A(nn.Module):
    def __init__(self, input_nc, output_nc, ngf=64, n_blocks=5):
        super(CalimarGan_Generator_A, self).__init__()
    ...
    def forward(self, input):
        x = F.pad(input[:, 0:1, :, :].repeat(1, 3, 1, 1), (3, 3, 3, 3), 'reflect') # x, x, x
        ...
        a = F.pad(input[:, 2:5, :, :], (3, 3, 3, 3), 'reflect') # x_LI, A_MG, A_MG
        a = F.relu(self.conv1_norm(self.conv1(a)))
        a = F.relu(self.conv2_norm(self.conv2(a)))
        a = F.relu(self.conv3_norm(self.conv3(a)))
        x_attention = F.relu(self.deconv1_norm_attention(self.deconv1_attention(a)))
        ...
        output10 = input[:, 2:3, :, :].repeat(1, 3, 1, 1) * attention10
...
class CalimarGan_Generator_B(nn.Module):
    # initializers
    def __init__(self, input_nc, output_nc, ngf=64, n_blocks=9):
        super(CalimarGan_Generator_B, self).__init__()
        self.input_nc = input_nc
        self.output_nc = output_nc
        self.ngf = ngf
        self.nb = n_blocks
...
class resnet_block(nn.Module):
    ...
    def forward(self, input):
        ...
        return input + F.relu(x)  # MODIFIED RS
